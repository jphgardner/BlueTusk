...
| 7. Continuous Graph preview | Implemented as capability-guarded registered SQL/PGQ plans with dependency-aware invalidation, authoritative `GRAPH_TABLE` requery/diff, samples, an optional Control Plane adapter, and benchmarks. | [compiler tests](../tests/BlueTusk.ContinuousGraph.Tests/ContinuousGraphQueryCompilerTests.cs), [PostgreSQL 19 integration](../tests/BlueTusk.ContinuousGraph.Tests/ContinuousGraphQueryIntegrationTests.cs), and the [Continuous Graph guide](continuous-graph/README.md). | Preview only and gated behind Control Plane; incremental graph maintenance remains explicitly out of scope. |
...
no vulnerable direct or transitive NuGet dependencies, validates 250
