...
`PostgreSqlRelayConsumerGroupSession` extends that lease across an initial snapshot. Acquire it before exporting the snapshot so retention cannot pass the group while rows are copied. `BeginSnapshotRunAsync` records the new epoch and consistent LSN; `CompleteSnapshotRunAsync` is called only after the destination durably completes the snapshot. The progress payload is versioned, bounded, and SHA-256 integrity checked. A reserved run left by a crash is abandoned when the next epoch begins, while a completed run supplies the baseline needed to resume CDC without resetting the destination.

Transactions at or below a completed snapshot's consistent LSN may be acknowledged without reapplying them because the authoritative snapshot already contains that state. Transactions above it must still pass through the normal destination-durability-before-acknowledgement path. `BlueTusk.Sync.DependencyInjection` implements this orchestration in `PostgreSqlRelaySyncPipelineSource`; setting a new group to `Latest` is never a substitute for the snapshot reservation protocol.
