...
public sealed class StreamsFormatCompatibilityTests
{
    [Fact]
    public void Registry_matches_implementation_and_named_evidence()
    {
        var repositoryRoot = FindRepositoryRoot();
        var registryPath = Path.Combine(repositoryRoot, "eng", "streams-formats.json");
        ...
        {
            ["change-transaction-envelope"] = (
                ChangeTransactionEnvelope.CurrentFormatVersion,
                ChangeTransactionEnvelope.MinimumSupportedFormatVersion,
                "backward-readable"),
            ["change-stream-checkpoint"] = (
                ChangeStreamCheckpoint.CurrentFormatVersion,
                ChangeStreamCheckpoint.CurrentFormatVersion,
                "backward-readable"),
            ["file-state-store"] = (
                FileChangeStreamStateFormat.CurrentVersion,
                FileChangeStreamStateFormat.CurrentVersion,
                "current-only"),
            ["postgresql-relay-schema"] = (
                PostgreSqlDurableChangeRelay.CurrentSchemaVersion,
                1,
                "migrated-in-place"),
            ["relay-backup"] = (
    ...
    private static void AssertEvidenceExists(string repositoryRoot, string evidence)
    {
        ...
        var path = Path.Combine(repositoryRoot, parts[0].Replace('/', Path.DirectorySeparatorChar));
