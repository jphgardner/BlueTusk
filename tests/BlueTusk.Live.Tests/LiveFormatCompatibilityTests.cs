...
public sealed class LiveFormatCompatibilityTests
{
    [Fact]
    public void Registry_matches_implementation_and_named_evidence()
    {
        var repositoryRoot = FindRepositoryRoot();
        var registryPath = Path.Combine(repositoryRoot, "eng", "live-formats.json");
        ...
        {
            ["grpc-contract"] = (1, 1, "additive-protobuf"),
            ["live-replay-json"] = (
                LiveReplayJsonSerializer.CurrentFormatVersion,
                LiveReplayJsonSerializer.MinimumSupportedFormatVersion,
                "current-only"),
            ["live-resume-token"] = (
                LiveResumeTokenProtector.CurrentFormatVersion,
                LiveResumeTokenProtector.MinimumSupportedFormatVersion,
                "current-only"),
            ["postgresql-live-schema"] = (
                PostgreSqlLiveInvalidationStore.CurrentSchemaVersion,
                PostgreSqlLiveInvalidationStore.MinimumSupportedSchemaVersion,
                "migrated-in-place"),
        };
        ...
        var replayVersion = LiveReplayJsonSerializer.CurrentFormatVersion.ToString(
            System.Globalization.CultureInfo.InvariantCulture);
        Assert.EndsWith(";v=" + replayVersion, LiveReplayJsonSerializer.ContentType);
        Assert.Contains(
            $"package bluetusk.live.v{expected["grpc-contract"].Current};",
            File.ReadAllText(
                Path.Combine(
                    repositoryRoot,
                    "src",
