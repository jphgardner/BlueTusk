...
internal static class WebhookSyncEnvelopeCodec
{
    internal static byte[] EncodeProvision(
        string deliveryId,
        ...
        Action<Utf8JsonWriter> writeEvent)
    {
        ...
        using (var writer = new Utf8JsonWriter(buffer))
        {
            ...
            writer.WriteNumber("formatVersion", WebhookSyncProtocol.CurrentFormatVersion);
        ...
        if (buffer.WrittenCount > maximumBytes)
        {
            throw new WebhookSyncProtocolException(
                $"The {buffer.WrittenCount}-byte webhook envelope exceeds the configured {maximumBytes}-byte limit.");
        }
