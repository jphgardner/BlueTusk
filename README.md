...
> [!IMPORTANT]
> BlueTusk is an experimental `0.3.0-preview.1` provider, not a production-ready database driver. Executable tests currently cover pooled ADO.NET queries, prepared statements, batches, streaming APIs, PostgreSQL-native types, Client-layer PostgreSQL pipeline mode, a `citext` codec and separately packaged EF integration preview plus authoring template and compatibility harness, replication preview APIs, EF Core CRUD, table CHECK and exclusion constraints, advanced PostgreSQL indexes, table/view/event-trigger, rewrite-rule, logical-publication/subscription, foreign-data-wrapper/server/user-mapping/foreign-table, tablespace, operator/operator-family/operator-class/cast/aggregate, declarative table-partition, row-level-security, direct table-inheritance, collation, installed-extension, enum/domain/composite/range/multirange-type, function/procedure, and ordinary/materialised-view migrations/scaffolding, PostgreSQL-native EF mappings plus typed native operators including quantified and row-value comparisons, scalar-function subsets including date/time and geometric families, aggregate-function subsets including JSON/XML/bitwise/statistical families, lateral array-expansion, typed `generate_series`, JSONB set-returning and model-derived recordset roots, initial multi-array `unnest` subsets, and model-registered user-defined table functions, and PostgreSQL 19 SQL/PGQ raw SQL, schema discovery/tooling, graph-aware migrations/reverse engineering, and a typed linear-path EF query subset. The measured transport evaluation retains ArrayPool/Span/Memory rather than adding `System.IO.Pipelines` to production packages. Stable extension APIs and the full production gate remain planned. Track exact implemented and pending scope in the [roadmap](docs/roadmap.md).
...
- EF Core CRUD, transactions, generated values, core LINQ, table CHECK and exclusion constraints, advanced column/expression PostgreSQL indexes, table/view/event-trigger, rewrite-rule, logical-publication/subscription, foreign-data-wrapper/server/user-mapping/foreign-table, tablespace, operator/operator-family/operator-class/cast/aggregate, declarative partition, row-level-security, direct table-inheritance, collation, installed-extension, enum/domain/composite/range/multirange-type, function/procedure, and ordinary/materialised-view migrations/scaffolding, typed PostgreSQL
  operator predicates including `ANY`/`ALL`, row-value comparisons, complete range/multirange positional forms, strict network containment, and full-text-query containment, array/range/JSONB/regex/network/full-text/date-time/geometric scalar functions, array/string/boolean/range/JSON/XML/bitwise, JSON-object, paired statistical/regression, ordered-set, and hypothetical-set aggregates, lateral array expansion, typed `generate_series`, scalar/key-value/model-derived JSONB roots, typed two- through four-array `unnest`, model-registered table-valued functions, initial migrations, and reverse engineering;
