...
- EF Core CRUD, transactions, generated values, core LINQ, typed PostgreSQL
  operator predicates including `ANY`/`ALL` and row-value comparisons, scalar functions, initial native aggregates, lateral array expansion, and typed integer, bigint, numeric, timestamp, and timestamp-with-time-zone `generate_series` query roots, initial migrations and reverse engineering;
